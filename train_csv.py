...
class CSVChessDataset(Dataset):
    ...
    def _preload_all(self):
        ...
        for i, fen in enumerate(self.fens):
            try:
                # Ensure fen is a clean string
                fen_str = str(fen).strip()
                # Remove quotes if present
                if fen_str.startswith('"') and fen_str.endswith('"'):
                    fen_str = fen_str[1:-1]
                if fen_str.startswith("'") and fen_str.endswith("'"):
                    fen_str = fen_str[1:-1]
                ...
                if not first_error_printed:
                    print(f"\n❌ FAILURE DETECTED ON ROW {i}!")
                    print(f"   Shape of bad FEN: {repr(fen)}")
                    print(f"   Cleaned FEN: {repr(fen_str)}")
